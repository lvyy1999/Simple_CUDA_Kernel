    ...
    for(int k0 = 0; k0 < K; k0 += BK) {
        ...
        for(int k = 0; k < BK; k++) {
            ...
            for(int i = 0; i < TM; i += 4) { 
                FLOAT4(At[i]) = FLOAT4(As[k][4 * rc + (i / 4) * 4 * C_BLOCK_ROWS]); // 向量化加载共享内存
            }
            #pragma unroll
            for(int j = 0; j < TN; j += 4) {
                FLOAT4(Bt[j]) = FLOAT4(Bs[k][4 * cc + (j / 4) * 4  * C_BLOCK_COLS]); // 向量化加载共享内存
            }
            // 在寄存器上做外积
            for(int i = 0; i < TM; i++) {
                #pragma unroll
...
extern "C" void gemm_v6(const half* A, const half* B, half* C, int M, int N, int K, float alpha, float beta) {
    ...
    gemm_kernel_v6<256, 128, 128, 8, 8, 8><<<blocksPerGrid, threadsPerBlock>>>(A, B, C, M, N, K, alpha, beta);
}
